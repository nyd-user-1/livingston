...
## 2026-08-30 00:20 ET — the fleet (lane FT, run by the lead)

Brendan at 22:05: "go at max speed until you have gone through all 50 states … prioritize bulk, then API in big batches, then HTML, then PDFs into S3 and run them all at once … no more stopping to wait for me." Built and running, in order:
- **`--shard i/k`** (`document_id % k`), **adaptive lanes** (`POLITE_AUTO_LANES=4:16`: +4 after 300 clean answers, halve on 429), **PDF deferral** (`PDF_DEFER_BUCKET` → `s3://livingston-bill-pdfs-638175140432/pdf/<state>/<id>.pdf`, row `pdf-deferred:`), **`--source pdf-batch`**, **`fleet-launch.sh`** (`--bootstrap` from stock Ubuntu 24.04 arm64 + the repo tarball in S3 — box 1's 100 GB AMI never finished snapshotting; `--pdf-batch` for the converter).
- **20 shards:** box 1 = shard 0, 19 × `t4g.medium` = shards 1–19. **Converter:** one `c7g.4xlarge` (`fleet=pdf`), 500 PDFs per 6–7 s. **Box 2:** Hawaii on its own (`data.capitol.hawaii.gov` — the old host 403s everyone, a rewrite fixed it) at ~140k/hour, plus the FEC tail.
- **Bugs found and fixed tonight, in order:** (1) `prepareSchema`'s `ALTER TABLE … IF NOT EXISTS` takes an exclusive lock even as a no-op — 320 fleet processes queued on it and every writer stalled (338 `Lock: relation` waits); it now checks first. (2) The shard selection `ORDER BY session_id DESC` made Neon sort a state's whole document set per round; unordered in shard mode. (3) 16 states × 400 docs per box = 320 concurrent selects on a 128 MB-buffer compute, 80–300 s each; now 4 × 4,000. (4) The driver's start-up "outstanding per state" count timed out on the saturated compute and crashed three shards before their first round (a dead driver terminates its box); shard mode now orders by an indexed `Bills` count with retries. (5) `http://` links that 404/fail get one retry over `https://` (LA, UT, NH, MI). (6) Ohio's retired solarapi v1 links → v2 API. (7) A parked PDF counts as progress for the driver's empty-round rule. (8) Partial index `billtexts_deferred_idx` for the converter's selection (7.5 min to build, concurrently).
- **The lead's own mistake:** restarting fleet drivers by killing the node process — the boot wrapper reads "driver exited" as "shard done" and terminates the box. Lost all 19 at 23:20 ET; relaunched in 4 minutes. Rule now in the launcher: kill the wrapper first.

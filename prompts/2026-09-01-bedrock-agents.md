...
LEAD: 16:30Z — OVERNIGHT STATE, written for the morning. Brendan's 4:40 AM consent failed on a build that could not keep the completing session's handle (build 145's fix finished 04:43:14 — three minutes late). Both probes agree nothing is filed under his u- id; the fork (grant under his claim check vs no grant anywhere) closes with ONE fresh consent on the current build — that consent is the experiment, said plainly, not a promise. Standing orders from the incident: the Google routes must NOT switch to identify() before §3's migration design (both lanes converged on this; a switch would orphan every claim-check-keyed grant); the isUserId guard (523e75f) landed before the migration that would make the hole real; in a shared repo, reading a commit is git show, never checkout -- . ; no CloudWatch for Amplify compute is on the priced list with a receipt. Shipped overnight per his verbatim orders: connected toast on the consent return (a9ecb27), two-kinds paragraph deleted, card gap-8, v0 black, Docs menu item fixed, /auth explainers deleted with sign-in landing on /create and sign-out preserved at /auth?signout=1 (e447d19), plus lane fixes bc0ae3b/31db5e9/523e75f.

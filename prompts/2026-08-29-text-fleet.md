...
**17:25 ET (lead) — sink drained to zero; 90.0 % (1,916,065 with text · 212,784 without · 2,128,849).**
The loader (post-`fccba32`) took the sink from 36,332 stubs to 48 in 33 rounds at ~20 rows/s with no failures — then spun on those 48: all Arizona, all pointing at `text/AZ/20260830/box-1788078312020-48.jsonl.gz`, an object that holds 48 *other* Arizona documents. Cause: the sink key was `box-<ms>-<rows>` with no per-writer entropy; two AZ shards flushed 48-row batches in the same millisecond (08:25:12.020Z) and the second PUT overwrote the first. The only such collision in the run. Fix `1e74f90`: the key now carries `pid` + 6 random hex; `runS3Load` deletes stubs whose document is not in their object (absence returns them to the fetch path) and clears the bill stamp where no text of it remains (8 of the 44 bills); the driver's `drain()` stops after three rounds that considered rows and changed none. Live: 48 stubs deleted, next round empty, `s3-load done`. The 44 bills were re-fetched from the Mac at azleg's 120 s crawl-delay: `48 considered · 48 stored · 23.9 min`, all 44 bills with text again. Parked PDFs: 0 (314 converted at 16:23 ET).
Noted, not acted on: `Documents.document_id` is shared between `text` and `supplement` rows (543650 is both an AZ 2012 bill text and a 2025 supplement) and `"BillTexts"` is keyed on `document_id` alone — harmless while only `document_type='text'` is fetched; a real collision risk if supplements are ever loaded into the same table.
Still running: box 2 only (FEC mirror ≥ 94 %, forms e/f); nothing on the Mac; no fleet boxes. The 10 % without text is unchanged in composition (IN token first).

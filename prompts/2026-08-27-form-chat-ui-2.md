...
Worker: Fable (lead session), 2026-08-28 ~03:30. Done directly on Brendan's instruction. Not committed at the time of writing; nothing verified in a browser (Brendan tests himself).

- Files changed: `src/lib/programs.ts`, `src/lib/form-fields.ts`, `src/lib/fill-form.ts` (3 lines), `src/components/ChatFormFields.tsx` (rewritten), `src/components/ChatMessage.tsx`, `src/components/ChatResponseFooter.tsx`, `src/components/FormProgress.tsx` (rewritten), `src/hooks/useChat.ts`, `src/types/chat.ts`, `src/pages/Chat.tsx`. New: `src/components/ui/select.tsx`. Dependency: `@radix-ui/react-select@2.3.7`.
- Item 1 — prompt: "one short line starting `**Section 17:**` … Not \"recorded\"". Client: `RECAP_LINE = /^Section\s+[\w–-]+:\s/` on the paragraph's text (`textOf(children)`); a green lucide `Check` is appended inside the `<p>`. Only in form mode (`form` prop set).
- Item 3 — `@radix-ui/react-select`; `ui/select.tsx` (trigger = input styling, chevron `mr-2`; content on `bg-popover shadow-popover`, items with `Check`). Enter-to-send: a keydown handler on the controls grid — Enter in an `<input>` (not textarea; Radix marks its own Enter as `defaultPrevented`) calls Send when it is enabled, or Save changes on a reopened question.
- Item 4 — `derivedFrom(messages, i)` in `Chat.tsx`: a user turn with no answers block whose next turn is a finished assistant turn → that turn's answers block (action keys dropped). Rendered under the bubble as `✓ Recorded:` + chips; the pencil opens the words AND the chips; Save calls `onEdit(null | text, changed)` — `null` leaves the message alone, `changed` merges into the record.
- Item 5 — `shelter.type` is now `rent, mortgage, roomAndBoard, trailerLot, shelter, none, other` (p.15 lines A–D, the page-2 shelter box, and "no housing cost"); `income[n].source` = the 27 printed kinds of §15 plus `job`/`selfEmployment` from §17 (30 values); `resources[n].kind` = the 21 printed kinds of §19; `education.highestGrade` = the 5 printed levels of §18 (was free text); `employment.status`, `maritalStatus`, `income[n].period`, `language.read/speak` gained `other`. Every one has a `…Detail` companion key; RECORDING rule added ("record `other` … never pick the nearest value"). Appendix `GROUPS` regexes are prefix-based and catch the Detail keys.
- Item 6 — `kind: "money"`: `$` adornment, thousands as typed (`formatMoney`), stored as digits + optional `.dd`. `kind: "tel"`: `(555) 555-5555` mask, stored as 10 digits. `kind: "email"`: domain lowercased on blur, red hairline only after blur. `displayValue(key, value)` in `programs.ts`: phone, money keys (`amount|value|cost|Paid|…`), DOB/lastWorked → `mm/dd/yyyy`, and fixed values → their printed label (`roomAndBoard` → `Room and board`). Used in chips, derived chips, the record list, the review table. Storage unchanged.
- Item 7 — prompt: `**Section 17 — Employment.**` / `**Section 17, continued.**` / unnumbered by title; "of 21" forbidden. `questionsOf` numbers questions within a section (`ordinal`/`count`) → collapsed rows read `Answered · 2 fields · Section 23 · 1 of 2`. Progress: `formProgress()` counts a section done when `#done` was said OR the interview has moved past it (the open question's section, or the furthest recorded key); the page adds sections whose every question is answered. Strip reads `Section 23 · 19 of 22 sections done` — 22 now, because the certification became a question section (item 8).
- Item 8 — Section 9 moved to the END of the `sections` array (after `vote`), `consent` removed, one `asks` line. `kind: "attest"` + `tone: "caution"` (house-forced for `certification.agree` in `form-fields.ts`, whatever the model wrote): amber box, `ATTESTATION` title, the statement from `help`, Yes/No chips (`yes|I agree`, `no|Not yet`), ⓘ top-right → `/forms/LDSS-2921.pdf#page=5` in a new tab. Prompt carries the softened statement verbatim and the "no is allowed" rule. `KEY_SECTION` maps `certification.` → 9.
- Item 9 — `voter.register` is house-forced `tone: "info"`: blue box, lucide `Vote`, ⓘ → page 27. Prompt: the pages-27–28 / filed-with-the-application / Board-of-Elections-mails-a-confirmation facts, "never say you emailed anyone or that they are registered". Pages 27–28 have **no detectable boxes** in `2921-checkboxes.json` (pages with boxes: 1,2,3,5,6,7,8,10,11,12,15,16,17,18,25) — the voter form's boxes are drawn differently; round-3 mapping work.
- Item 10 — form mode renders with `[remarkGfm]` only (no math plugins); corpus chat unchanged. Table components added for both chats, with a hover copy icon (TSV). ```livingston-review``` → `ReviewTable` drawn from `record` (one row per asked section, `labelFor: displayValue` cells, `skip`/`unknown` → `—`), copy icon. Prompt example placeholders: `<first name as they gave it>` / `<YYYY-MM-DD>`. THE CLOSE section: one sentence, the review block, the fill question — "Do not list the answers yourself".
- Item 11 — `form.fill` in FORM_KEYS (options `yes|Yes, fill it out` / `no|Not yet`); `isActionKey()` keeps it out of the record and the RECORDED block. Yes → `sendMessage(answers)` first, then `appendMessage("form-pdf")` so the order is user / reply / form; `buildPdf(id)` runs `fillForm` and stores `{url, bytes}` in page state keyed by message id. `ChatMessage kind="form-pdf"` renders `FormPdfPanel`: spinner row → iframe on the blob (`#page=1&view=FitH`) with `Open in new tab`, then `FormDelivery`. Persisted as `kind: "form-pdf"` with placeholder content; excluded from the history sent to the model; on reload every form-pdf turn is rebuilt from the record on mount. "Put it on the form" in the strip still works. The model's spoken "yes" path is covered too (`fillIfAsked` on the harvested block).
- Item 12 — table styling applies to the corpus chat as well.
- Build result: `npx vite build` clean; `npx eslint` clean on all 17 touched files.
- What you verified visually vs. in the built output: nothing visually. Bundled `programs.ts` with esbuild and ran it: prompt 20.7k chars with THE CERTIFICATION / VOTER REGISTRATION / THE CLOSE / CITING present; `displayValue` → `(555) 555-5555`, `$1,200`, `01/01/1980`, `Room and board`; `formProgress` on a 4-key record at Section 23 → 19 of 22; `form.fill` absent from RECORDED; rail order Public Assistance / Childcare Assistance / Earned Income Tax Credit / HEAP / SNAP.
- Open items / disagreements: none. Note for Q/A: the section count is 22 now (21 + the certification).

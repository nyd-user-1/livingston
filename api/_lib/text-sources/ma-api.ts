...
/** …/Bills/194/S1247.pdf | …/Bills/186/House/H1648 -> { court: 194, bill: "S1247" } */
export function parseMaLink(link: string): { court: number; bill: string } | null {
  const m = /malegislature\.gov\/Bills\/(\d+)\/(?:House\/|Senate\/)?([HS]D?\d+)(?:\.pdf)?\/?$/i.exec(link);
...
export async function runMaApi(sql: Sql, opts: MaOpts, counts: Counts) {
  const rows = (await sql.query(
    `SELECT d.document_id, d.bill_id, d.document_desc, d.state_link, d.document_mime, b.session_id
      ...
      WHERE b.state = 'MA' AND d.document_type = 'text' AND coalesce(d.state_link, '') <> ''
        AND (t.document_id IS NULL OR (t.text IS NULL AND t.error LIKE 'ma-api: HTTP%'))
      ORDER BY b.session_id DESC, d.bill_id, d.document_id
      LIMIT $1`,
    [opts.limit],
  )) as { document_id: number; bill_id: number; document_desc: string; state_link: string; document_mime: string; session_id: number }[];
  ...
  const worker = async () => {
    for (;;) {
      ...
      const p = parseMaLink(d.state_link);
